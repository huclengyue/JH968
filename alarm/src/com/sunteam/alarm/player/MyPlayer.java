package com.sunteam.alarm.player;
...
public class MyPlayer implements OnCompletionListener, OnErrorListener {
	...
	private void startPlay(float percentage, String path) {
		...
		try {
			...
			mPlayer.prepare();
			mPlayer.start();
			// Global.debug("\r\n startPlay (int) (percentage *
			// mPlayer.getDuration()) == " + (int) (percentage *
			// mPlayer.getDuration()));
			// mPlayer.seekTo((int) (percentage * mPlayer.getDuration())); //
			// baocaho del mp2 文件播放时出现
	...
	public void rewind() {
		if (state() == MyPlayer.PLAYING_STATE) {
			...
			pos -= 10000;// 快退 10s
			if (pos <= -8000) {
				// Global.showToast(mcontext, R.string.rewind2start,
				// mHandler,0);
				stopPlayback();
			} else {
				mPlayer.seekTo(pos);
