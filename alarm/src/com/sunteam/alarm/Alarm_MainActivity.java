...
public class Alarm_MainActivity extends MenuActivity {
	...
	//				intent.setAction("set_action");// 启动设置界面
			...
			else if(gInterfaceFlag == Global.ANNIVERSARY_INFO_INTERFACE){  // 纪念日详情 界面
				if(Global.ANNIVERSARY_SET_DATE == getSelectItem() ||
						Global.ANNIVERSARY_SET_TIME == getSelectItem()){
					StartSetting(getSelectItem());
				}
				else{
					StartSettingList(getSelectItem());
...
/*		
		...
		gMonth = calendar.get(Calendar.MONTH) + 1; // 月份从 0开始
		gDay = calendar.get(Calendar.DAY_OF_MONTH);
*/		
		temp += getResources().getString(R.string.date) + gMonth +
				getResources().getString(R.string.month) + gDay + getResources().getString(R.string.day);
