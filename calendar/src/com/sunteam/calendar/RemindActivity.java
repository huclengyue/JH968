...
public class RemindActivity extends MenuActivity {
	...
	private String gFileName = Global.ALARM_FILE_NAME; // Alarmpublic.ALARM_FILE_NAME;
														// // 录音文件名
														// getResources().getString(R.string.remind_noFile);
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		...
		if (keyCode == KeyEvent.KEYCODE_BACK) { // 返回
			if (gFileName.equals(Global.ALARM_FILE_NAME)) { // 没有保存记录
				...
				Global.debug("\r\n 可以保存 ------------------- \r\n");
				boolean rtn = false;
				if (gCall_Flag == Global.REMIND_CALL_MENU) { // 菜单界面进入
	...
	private void startRecord() {
		...
		String path = Global.TIXING_PATH; // Environment.getExternalStorageDirectory().getAbsolutePath()
											// + "/tixing";
		gPath = path;
		Date dt = new Date();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == Global.FLAG_RECORD_ID /*
													 * && resultCode ==
													 * Global.FLAG_RECORD_ID
													 */) { // 录音界面返回
	...
	private Boolean saveDataRemind() {
		...
		if (max_num <= 0) // 无数据
		{
			...
			for (int i = 0; i < max_num; i++) // 查找相同的 替换
			{
				...
				if ((tempinfo.year == tempinfo_1.year) && (tempinfo.month == tempinfo_1.month)
						&& (tempinfo.day == tempinfo_1.day) && (tempinfo.hour == tempinfo_1.hour)
						&& (tempinfo.minute == tempinfo_1.minute)) // 同一天
				{
					tempinfo._id = tempinfo_1._id;
					ConfirmDialog mConfirmDialog = new ConfirmDialog(this,
							getResources().getString(R.string.save_update), getResources().getString(R.string.ok),
	...
	private Boolean saveDataRemindForMenu() {
		...
		Global.debug("saveDataRemindForMenu ===111=== tempinfo.onoff ==" + tempinfo.onoff);
		Calendar calendar = Calendar.getInstance(); // 获取日历
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == Global.MSG_START_RECORD) { // 开始录音
				...
				saveUpdateNo();
			} else if (msg.what == Global.MSG_PLAY_RECORD){
				playRecordSound();
			}
