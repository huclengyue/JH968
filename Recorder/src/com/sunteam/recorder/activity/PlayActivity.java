...
public class PlayActivity extends RecordBaseActivity implements MyPlayer.OnStateChangedListener {
	...
	private void updateTimerView() {
		if (myPlayer.state() == MyPlayer.PLAYING_STATE) {
			...
			if (total >= 3600) {
				tvPlayedTime
						.setText(String.format(mTimerFormatHMS, current / 3600, (current % 3600) / 60, current % 60));
			} else {
			...
			if (intent.getAction().equals(Intent.ACTION_BATTERY_CHANGED)) {
				...
				if (level < 10) {
					if (myPlayer.state() == MyPlayer.PLAYING_STATE) {
						...
						// myPlayer.pausePlayback();
						PromptDialog mPromptDialog = new PromptDialog(PlayActivity.this,
								getResources().getString(R.string.cannot_play));
						mPromptDialog.show();
						mPromptDialog.setPromptListener(new PromptListener() {
