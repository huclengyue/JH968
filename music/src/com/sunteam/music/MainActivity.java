...
public class MainActivity extends MenuActivity implements ShowView {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		selectItem = SharedPrefUtils.getSharedPrefInt(this, Global.MUSIC_CONFIG_FILE, Context.MODE_WORLD_READABLE,
				Global.MUSIC_SELECT, selectItem);
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		if (KeyEvent.KEYCODE_MENU == keyCode) { // 按键 menu
			...
			if (intface_flag == FAVORITE_INTFACE) { // 在我的最爱界面
				...
				// startMenu(0, gFilePaths.get(getSelectItem()),
				// gFileName.get(getSelectItem()));
				startMenu(0, gFilePaths.get(getSelectItem()), gFileName.get(getSelectItem()));
			} else if (intface_flag == DIRECTORY_1_INTFACE) { // 可以添加
				Global.debug("\r\n 【MainActivity】 ----> onKeyUp ========KEYCODE_MENU mMenuList.size() == "
						+ mMenuList.size());
				if (mMenuList.size() <= 0) {
					return true;
				}
				...
				gId = getSelectItem();
				// startPlayMenu(0, gFilePaths.get(getSelectItem()),
				// gFileName.get(getSelectItem()));
	...
	private void keyupBack() {
		...
		if (MAIN_INTFACE == intface_flag) { // 主界面 直接退出
			finish();
				...
				DIRECTORY_INTFACE == intface_flag || RECENTPLAY_INTFACE == intface_flag) {
			gFileName.clear();
			gFilePaths.clear();
	...
	private void keyupEnter() {
		...
		if (MAIN_INTFACE == intface_flag) { // 主界面
			if (getSelectItem() == Global.MAIN_DIR_ID) { // 选中第一项
				SharedPrefUtils.setSharedPrefInt(this, Global.MUSIC_CONFIG_FILE, Context.MODE_WORLD_READABLE,
						Global.MUSIC_SELECT, getSelectItem());
				intface_flag = DIRECTORY_INTFACE; // 进入目录浏览界面
			...
			} else if (getSelectItem() == Global.MAIN_SAVE_ID) { // 选中第二项 我的最爱界面
																	// 显示列表
				SharedPrefUtils.setSharedPrefInt(this, Global.MUSIC_CONFIG_FILE, Context.MODE_WORLD_READABLE,
						Global.MUSIC_SELECT, getSelectItem());
			...
			} else if (getSelectItem() == Global.MAIN_PLAY_ID) { // 选中第三项 最近播放界面
																	// 显示列表
				SharedPrefUtils.setSharedPrefInt(this, Global.MUSIC_CONFIG_FILE, Context.MODE_WORLD_READABLE,
						Global.MUSIC_SELECT, getSelectItem());
		...
		// 目录浏览界面
			...
			if (getSelectItem() == Global.MAIN_LOCAL_ID) { // 选中第一项 进内存
				...
				// gFileName = ArrayUtils.strArray2List
				// (getResources().getStringArray(R.array.main1_list));
				// setTitle(getSelectItemContent());
				...
				if (true == getExtSDUSBPathHave(Global.USER_PATH)) // 存储卡存在
				{
					...
					PromptDialog mPromptDialog = new PromptDialog(this,
							getResources().getString(R.string.error_no_sdcard));
					mPromptDialog.show();
					mPromptDialog.setPromptListener(new PromptListener() {
				...
				if (true == getExtSDUSBPathHave(Global.USB_PATH)) // 优盘存在
				{
					...
					PromptDialog mPromptDialog = new PromptDialog(this,
							getResources().getString(R.string.error_no_udisk));
					mPromptDialog.show();
					mPromptDialog.setPromptListener(new PromptListener() {
		...
		} else if (FAVORITE_INTFACE == intface_flag || FAVORITE_1_INTFACE == intface_flag) // 我的最爱界面
		{
			...
			Global.debug("\r\n [MainActivity] gFilePaths.get(getSelectItem()) =2== " + gFilePaths.get(getSelectItem())
					+ "/" + gFileName.get(getSelectItem()));
			File mFile = new File(gFilePaths.get(
					getSelectItem()) /* +"/"+gFileName.get(getSelectItem()) */); // 获取路径内容
			// Global.debug("[@@@] gFilePaths.size() ===="+ gFilePaths.size());
			if (mFile.isDirectory()) { // 是文件夹
				...
				// showList(gFilePaths.get(getSelectItem())+"/"+gFileName.get(getSelectItem()),
				// getSelectItemContent());
				intface_flag = FAVORITE_1_INTFACE;
				onResume();
			} else { // 是文件
						// getPathAudioList(gPath.get(gPathNum -1));
						// Global.debug("1111111111111111 gFilePaths.size() =="+
						// gFilePaths.size());
				gPlayListName.clear();
				// Global.debug("111111122211111 gFilePaths.size() =="+
				// gFilePaths.size());
				...
				// Global.debug("2222222222222222 gFilePaths.size()== "+
				// gFilePaths.size());
				for (int i = 0; i < gFileName.size(); i++) {
					...
					// File mFile1 = new
					// File(gFilePaths.get(i)+"/"+gFileName.get(i)); // 获取路径内容
		...
		} else if (DIRECTORY_1_INTFACE == intface_flag) // 目录浏览界面
		{
			File mFile = new File(gFilePaths.get(getSelectItem())); // 获取路径内容
			// File mFile = new
			// File(gFilePaths.get(getSelectItem())+"/"+gFileName.get(getSelectItem()));
			// // 获取路径内容
			if (mFile.isDirectory()) { // 是文件夹

				// String path =
				// gFilePaths.get(getSelectItem())+"/"+gFileName.get(getSelectItem());
				// 保存反显 返回时 需要
				gSelecyId[gPathNum] = getSelectItem();
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if (KeyEvent.KEYCODE_MENU == keyCode) { // 按键 menu
			...
			return true;
		} else if (KeyEvent.KEYCODE_DPAD_DOWN == keyCode || KeyEvent.KEYCODE_DPAD_LEFT == keyCode
				|| KeyEvent.KEYCODE_DPAD_RIGHT == keyCode || KeyEvent.KEYCODE_DPAD_UP == keyCode) {
			if (mMenuList.size() <= 0) {
				PromptDialog mPromptDialog = new PromptDialog(this, getResources().getString(R.string.no_file));
				mPromptDialog.show();
	...
	public View getView(Context context, int position, View convertView, ViewGroup parent) {
		...
		if (gFilePaths != null) {
			...
			// File mFile = new File(gFilePaths.get(position).toString()); //
			// 获取当前路径
			// String path =
			// gFilePaths.get(position).toString()+"/"+gFileName.get(position).toString();
			...
			} else if ((mFile.isDirectory()) || path.equals(Global.DIRECTORY_PATH) || path.equals(Global.FAVORITE_PATH)
					...
					for (File mCurrentFile : mFiles) {
						...
						if (mFile1.isDirectory()) { // 文件夹
							if (true == getDirectoryhaveMP3(mCurrentFile.getPath())) { // 文件夹内有音频文件
								...
								if ((Global.FristString != null)
										&& (Global.FristString.contains(mCurrentFile.getPath()))) {
									selectId = allId;
							...
							String prefix = getExtensionName(mCurrentFile.getName()); // 获取文件名
																						// 后缀
							if (prefix.equals("mp3") || prefix.equals("MP3") || prefix.equals("wav")
									|| prefix.equals("WAV") || prefix.equals("wma") || prefix.equals("WMA")
									|| prefix.equals("aac") || prefix.equals("AAC") || prefix.equals("amr")
									|| prefix.equals("AMR") || prefix.equals("flac") || prefix.equals("FLAC")
									|| prefix.equals("m4a") || prefix.equals("M4A") || prefix.equals("m4r")
									|| prefix.equals("M4R") || prefix.equals("MID") || prefix.equals("mid")
									|| prefix.equals("mp2") || prefix.equals("MP2") || prefix.equals("ogg")
									|| prefix.equals("OGG") ||
									...
									// prefix.equals("rmvb") ||
									// prefix.equals("RMVB")||
									prefix.equals("wmv") || prefix.equals("WMV") || prefix.equals("mkv")
									|| prefix.equals("MKV") || prefix.equals("flv") || prefix.equals("flV")
									|| prefix.equals("asf") || prefix.equals("ASF") || prefix.equals("3gp")
	...
	private boolean showDbList(int flag) {
		...
		for (int i = 0; i < musicInfos.size(); i++) {
			...
			// File mFile = new File(musicinfo.path+"/"+musicinfo.filename ); //
			// 获取路径内容
		...
		// Global.debug("\r\n[**] mSelectName == "+ mSelectName + " mselectId
		// ==" + mselectId);
				...
				for (File mCurrentFile : mFiles) {
					...
					if (mFile2.isFile()) { // 是文件
						...
						if (prefix.equals("mp3") || prefix.equals("MP3") || prefix.equals("wav") || prefix.equals("WAV")
								|| prefix.equals("wma") || prefix.equals("WMA") || prefix.equals("aac")
								|| prefix.equals("AAC") || prefix.equals("amr") || prefix.equals("AMR")
								|| prefix.equals("flac") || prefix.equals("FLAC") || prefix.equals("m4a")
								|| prefix.equals("M4A") || prefix.equals("m4r") || prefix.equals("M4R")
								|| prefix.equals("MID") || prefix.equals("mid") || prefix.equals("mp2")
								|| prefix.equals("MP2") || prefix.equals("ogg") || prefix.equals("OGG") ||
	...
	public void MusicAddSaveList() {
		...
		if (max_id <= 0) { // 无数据
			...
			for (int i = 0; i < mMusicInfos.size(); i++) {
				...
				// String mFilePath1 =
				// musicinfo.getPath()+"/"+musicinfo.getFileName();
