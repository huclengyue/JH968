...
//	private static final String DBNAME = "massage.db";// 数据库名称
	private static final String table_cmd = " (_id integer primary key, filename string, path string , playtime integer)";   // 最近播放 
	...
	public void onCreate(SQLiteDatabase db){// 首先建立每个数据的数据表
		...
		try {
			...
			String cmd = "create table "+ Global.PLAY_LIST + table_cmd; // 创建 最近播放表
			db.execSQL(cmd); // 创建 表
			
			cmd = null;
			cmd = "create table "+ Global.SAVE_LIST + table_cmd; // 创建 我的收藏表
			db.execSQL(cmd); // 创建 表
