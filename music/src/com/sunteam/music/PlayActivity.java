...
public class PlayActivity extends BaseActivity implements MyPlayer.OnStateChangedListener {
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == 1) { // 音乐播放结束消息
				/*
				 * Intent intent = new Intent(); intent.putExtra("currentIndex",
				 * currentIndex); intent.putExtra("filename",
				 * gPlayListName.get(currentIndex)); setResult(RESULT_OK,
				 * intent); //finish();
				 * 
				 */
				MusicAddPlayList();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 获取播放模式
		gPlay_Mode = SharedPrefUtils.getSharedPrefInt(this, Global.MUSIC_CONFIG_FILE, Context.MODE_WORLD_READABLE,
				Global.MUSIC_MODE, gPlay_Mode);
		...
		// gPlayListName.get(currentIndex);
		String FilePath = gPlayListPaths.get(currentIndex);
		...
		if (total <= 0) {
			...
			// (Global.FristString.equals(path))){
			...
			public void onClick(View arg0) {
				if (myPlayer.state() == MyPlayer.IDLE_STATE || myPlayer.state() == MyPlayer.PLAYING_PAUSED_STATE) {
					...
					if (total <= 0) { // 说明文件有
						...
						if (total >= 3600) {
							tvTotalTime.setText(
									String.format(mTimerFormatHMS, total / 3600, (total % 3600) / 60, total % 60));
						} else {
							tvTotalTime.setText(String.format(mTimerFormatMS, total / 60, total % 60));
						}
						updateUI();
			...
			if ((gAB_flag == true) && (current >= gAB_B)) {
				...
				if (total >= 3600) {
					tvPlayedTime.setText(
							String.format(mTimerFormatHMS, current / 3600, (current % 3600) / 60, current % 60));
				} else {
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		...
		if (keyCode == KeyEvent.KEYCODE_BACK) { // 退出
			...
			if ((gAB_A < 1) || (myPlayer.state() == MyPlayer.IDLE_STATE)
					|| (myPlayer.state() == MyPlayer.PLAYING_PAUSED_STATE)) {
				return true;
			}
			setPalyst(false);
		...
		if (flag == true) {
			if (myPlayer.state() == MyPlayer.IDLE_STATE || myPlayer.state() == MyPlayer.PLAYING_PAUSED_STATE) {
				...
				if (total <= 0) {
					...
					if (total >= 3600) {
						tvTotalTime
								.setText(String.format(mTimerFormatHMS, total / 3600, (total % 3600) / 60, total % 60));
					} else {
						tvTotalTime.setText(String.format(mTimerFormatMS, total / 60, total % 60));
					}
					updateUI();
	...
	public void onError(int error) {
		...
		if (message != null) {
			// Global.showToast(PlayActivity.this, R.string.error_file,
			// mHandler, 1);
		}
	}
	...
	public class BatteryBroadcastReciver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(Intent.ACTION_BATTERY_CHANGED)) {
				...
				// int
				// totalCapacity=intent.getIntExtra(BatteryManager.EXTRA_SCALE,
				// 100);
				...
				if (level < 10) {
					if (myPlayer.state() == MyPlayer.PLAYING_STATE) {
						// Global.showToast(PlayActivity.this,
						// R.string.cannot_play,mHandler,0);
						// myPlayer.pausePlayback();
						myPlayer.stopPlayback();
						PromptDialog mpDialog = new PromptDialog(PlayActivity.this,
								getResources().getString(R.string.bat_lv0));
						mpDialog.show();
	...
	public void MusicAddPlayList() {
		...
		if (max_id <= 0) { // 无数据
			...
			if (true == data_flag /*
									 * dbMusicInfo.hasRecord(Global.PLAY_LIST,
									 * musicinfo)
									 */) // 有此条记录
			{
				// dbMusicInfo.deteleForOneByPath(Filename, FilePath,
				// Global.PLAY_LIST_ID); // 删除此条记录
				// dbMusicInfo.deteleForOneByP(Filename, FilePath,
				// Global.PLAY_LIST_ID); // 删除此条记录
				dbMusicInfo.deteleForOne(musicinfo_temp._id, Global.PLAY_LIST_ID); // 删除
																					// 最小的
	...
	/*
	 * // 增加我的最爱 记录 public void MusicAddSaveList() { MusicInfo musicinfo = new
	 * MusicInfo(); //创建 结构体 GetDbInfo dbMusicInfo = new GetDbInfo( this ); //
	 * 打开数据库
	 * 
	 * String Filename = gPlayListName.get(currentIndex); String FilePath =
	 * gPlayListPaths.get(currentIndex); int max_id =
	 * dbMusicInfo.getMaxId(Global.SAVE_LIST_ID); Global.debug(
	 * "\r\n MusicAddPlayList ===== max_id = " + max_id); if(max_id <= 0){ //
	 * 无数据 musicinfo._id = 1; musicinfo.path = FilePath; musicinfo.filename =
	 * Filename;
	 * 
	 * dbMusicInfo.add(musicinfo , Global.SAVE_LIST_ID);
	 * TtsUtils.getInstance().speak(getResources().getString(R.string.
	 * file_add_ok)); } else{ boolean flag = false; ArrayList<MusicInfo>
	 * mMusicInfo = new ArrayList<MusicInfo>(); mMusicInfo =
	 * dbMusicInfo.GetAllData(Global.SAVE_LIST); Global.debug(
	 * "\r\n mMusicInfo.size() === "+ mMusicInfo.size()); for(int i = 0; i <
	 * mMusicInfo.size(); i++) { musicinfo = mMusicInfo.get(i); Global.debug(
	 * "\r\n musicinfo.path === "+ musicinfo.path + " FilePath =="+FilePath);
	 * if(FilePath.equals(musicinfo.getPath())){ // 文件重复 flag = true; break; } }
	 * 
	 * if(true == flag){ // 文件重复
	 * TtsUtils.getInstance().speak(getResources().getString(R.string.
	 * file_exists)); } else{ // 添加文件 musicinfo._id = max_id + 1; musicinfo.path
	 * = FilePath; musicinfo.filename = Filename; dbMusicInfo.add(musicinfo,
	 * Global.SAVE_LIST_ID);
	 * TtsUtils.getInstance().speak(getResources().getString(R.string.
	 * file_add_ok)); } } dbMusicInfo.closeDb(); }
	 */
	// 进入功能菜单
