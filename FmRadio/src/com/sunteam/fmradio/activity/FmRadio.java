...
/**
 * An example FM Receiver application that supports the following features:
 * <li>Access to the FM Receiver Service.
 * <li>Preset station management.
 * <li>RDS capability
 */
...
public class FmRadio extends MenuActivity implements IRadioViewRxTouchEventHandler, IFmProxyCallback {
	...
	private int mFrequencyStep = 10; // Step in increments of 100 Hz // 每次增加
										// 100hz
	...
	// private HeadsetPlugUnplugBroadcastReceiver
	...
	private int chanel_last = 0; // 上一次搜到的电台
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (false) { // 耳机插入
			...
			Global.debug("\r\n mTmpList ==== " + mTmpList);
			if (null == mTmpList) {
				// mTitle = DEFAULT_FREQUENCY/100+"."+
				// DEFAULT_FREQUENCY%100/10+"MHz";
	...
	private void Init() {
		 ...
		 * getResources().getString(R.string.stop_backplay),
		 * Toast.LENGTH_LONG).show();
		 * //TtsUtils.getInstance().speak(getResources().getString(R.string.
		 * music_is_playing)); finish(); }
		 */
		/*
		 * // 获取设备模式 在 飞行模式是 不可以打开收音机 if
		 * (Settings.System.getInt(getApplicationContext().getContentResolver(),
		 * Settings.Global.AIRPLANE_MODE_ON, 0) != 0) { Toast.makeText(this,
		 * "Cannot open FMRadio in Airplane mode", Toast.LENGTH_SHORT).show();
		 * bFinishCalled = true; Global.debug(
		 * "Airplane mode ====================\r\n"); finish(); }
		 */
		...
		mytimer.schedule(mytask, 3000, 3000); // 1秒后执行
	...
	// 声音控制
	private void doSoundEffect(boolean state) { // state : 0 apk 启动， 1 apk
												// 退出恢复soundeffect
		ContentResolver res;
		int ret;
...
//		Global.debug("\r\n  [onResume()]   =====END--=--- ");
	...
	protected void onDestroy() {
		...
		// Global.debug("FmRadio --------------ondestroy- go to set linein-
		// disable---");
	...
	private BroadcastReceiver mAirplaneModeReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(Intent.ACTION_AIRPLANE_MODE_CHANGED)) {
				...
				// Toast.makeText(getApplicationContext(), "Shutting down
				// FMRadio in Airplane mode",
	...
	private void powerUpSequence() {
		...
		// Global.debug("【开机】[powerUpSequence] radio... mFmReceiver = " +
		// mFmReceiver + " ; Softmute state:"
		...
		if (FmConstants.FM_SOFTMUTE_FEATURE_ENABLED) {
			...
			status = mFmReceiver.turnOnRadio(FmProxy.FUNC_REGION_NA | FmProxy.FUNC_RBDS | FmProxy.FUNC_AF,
					getPackageName());
		}
	...
	private void powerupComplete() {
		...
		// mPendingFrequency = mSharedPrefs.getInt(lastFreqPreferenceKey,
		// DEFAULT_FREQUENCY);
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		...
		if (keyCode == KeyEvent.KEYCODE_DPAD_RIGHT) { // 右键处理 频道+ 0.1
			// Global.debug("gheadin = " + gheadin + " mSearchFlag = " +
			// mSearchFlag + " mMenuList.size() =" + mMenuList.size());
			...
			return true;
		} else if (keyCode == KeyEvent.KEYCODE_DPAD_LEFT) { // 右键处理 // 右键处理 频道-
															// 0.1
			...
			return true;
		} else if (keyCode == KeyEvent.KEYCODE_DPAD_UP || keyCode == KeyEvent.KEYCODE_DPAD_DOWN) { // 上键处理
																									// 搜索上一台
			if ((gheadin == false) || (mSearchFlag == true) || (mMenuList.size() <= 0)) // 正在搜台时禁止按键
			{
				return true;
			 ...
			 * else if (keyCode == KeyEvent.KEYCODE_DPAD_DOWN) { // // 下键处理
			 * 搜索下一台 if ((gheadin == false) || (mSearchFlag == true) ||
			 * (mMenuList.size() <= 0)) // 正在搜台时禁止按键 { return true; }
			 * updateMuted(true); // 静音 //playThisRadio(); }
			...
			return true;
																									...
																									// 搜索上一台
			// Global.debug("\r\n[FmRadio]-->[onKeyUp]--->gheadin == " +
			// gheadin);
			// Global.debug("\r\n[FmRadio]-->[onKeyUp]--->mSearchFlag == " +
			// mSearchFlag);
			// Global.debug("\r\n[FmRadio]-->[onKeyUp]--->mMenuList.size() == "
			// + mMenuList.size());
			...
			if (mPendingMute == true) { //
				...
				mPromptDialog.setPromptListener(new PromptListener() {
					...
					public void onComplete() {
						...
						updateMuted(!mPendingMute);
						Message msg = Message.obtain();
						msg.what = Global.MSG_TTSDOCOMPLETES;
						msg.arg1 = chanel_num;
						viewUpdateHandler.sendMessage(msg);
					}
				});
		...
		} else if (keyCode == KeyEvent.KEYCODE_0 || keyCode == KeyEvent.KEYCODE_STAR
				|| keyCode == KeyEvent.KEYCODE_POUND) {
			Global.debug("\r\n[onKeyUp]   Test == " + Test);
			Test = !Test;
//			AudioManager mAudioManager = (AudioManager) getSystemService(Context.AUDIO_SERVICE);
//			Global.debug("\r\n[onKeyUp]   mAudioManager.isStreamMute(AudioSystem.STREAM_FM) == " +  mAudioManager.isStreamMute(10));
			//mAudioManager.isStreamMute(AudioSystem.STREAM_FM);
			return true;
		}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (selectItem == Global.SEARCH_ALL_ID) { // 全部搜索
			 ...
			 * 
			 * mRadioFreq = Global.DEFAULT_FREQUENCY; String s =
			 * String.format("%.1f", mRadioFreq / Global.fmScale); mTitle = s +
			 * getResources().getString(R.string.mhz);//"MHz";
			 * 
			 * chanel_str.add(mTitle); chanel_num++;
			 * 
			 * mMenuList = chanel_str; radioFreqList.add(mRadioFreq);
			 * 
			 * setListData(mMenuList); mMenuView.setSelectItem(0);
			 * 
			 * setTitle(mTitle); int freq = radioFreqList.get(0);
			 * 
			 * SharedPrefUtils.setSharedPrefInt(this,Global.FM_CONFIG_FILE,
			 * Context.MODE_WORLD_READABLE, Global.FM_SELECT, freq );
			 * updateFrequency(freq);
			 * 
			 * onResume();
			 */
	...
	private void startRecord() {
		...
		String path = Global.FM_PATH; // Environment.getExternalStorageDirectory().getAbsolutePath()
										// + "/tixing";
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if (keyCode == KeyEvent.KEYCODE_DPAD_RIGHT || keyCode == KeyEvent.KEYCODE_DPAD_LEFT
			...
			// Global.debug("\r\n [FmRadio] [onKeyDown] mMenuList.size() ==" +
			// mMenuList.size());
			if ((mSearchFlag == true) || (mMenuList.size() <= 0)) // 正在搜台时禁止按键
			{
				return true;
			...
			// Global.debug("\r\n[FmRadio]-->[onKeyDown]--->gheadin == " +
			// gheadin);
			// Global.debug("\r\n[FmRadio]-->[onKeyDown]--->mSearchFlag == " +
			// mSearchFlag);
			// Global.debug("\r\n[FmRadio]-->[onKeyDown]--->mMenuList.size() ==
			// " + mMenuList.size());
	...
	private void updateScanStep(SharedPreferences sp) {
		...
		// mPendingScanStep =
		// Integer.parseInt(mSharedPrefs.getString(FmRadioSettings.FM_PREF_SCAN_STEP,
		// "0"));
...
//		Global.debug("\r\n[FmRadio]-->[updateFMVolume] vol_bk === " + vol_bk);
	...
	private void updateAudioMode(SharedPreferences sp) {
		...
		/* Extract preferences and request these settings. */
		// mPendingAudioMode =
		// Integer.parseInt(mSharedPrefs.getString(FmRadioSettings.FM_PREF_AUDIO_MODE,
		// "0"));
	...
	/**
	 * Request new audio path operation and pend if necessary. 更新 状态
	 * 
	 * @param sp
	...
	/**
	 * Request frequency tuning operation and pend if necessary. 更新频率
	 * 
	 * @param freq
	...
	private void updateFrequency(int freq) {
		...
		// Global.debug("\r\n [updateFrequency] == mFmReceiver ==【 " +
		// mFmReceiver + "】 freq == " + freq);
	...
	/**
	 * Request mute operation and pend if necessary. 静音
	 * 
	 * @param muted
...
/*		if (muted == true) {
	...
	private void updateNflEstimate(SharedPreferences sp) {
		...
		/* Extract preferences and request these settings. */
		// mPendingNflEstimate =
		// Integer.parseInt(mSharedPrefs.getString(FmRadioSettings.FM_PREF_NFL_MODE,
		// "1"));

		Global.debug("Sending NFL mode (" + Integer.toString(mPendingNflEstimate) + ")");

		if (null != mFmReceiver) {
			nflEstimateUpdatePending = (FmProxy.STATUS_OK != mFmReceiver.estimateNoiseFloorLevel(mPendingNflEstimate));
		}
	}

	/**
	 * Request station search operation and pend if necessary. 搜索电台 参数 向上或向下搜索
	 * 更新搜索状态
	 * @param direction
...
//		Global.debug("[fmradio]--->[updateStationSearch]  ==========");
		...
		// Global.debug( "Sending search direction (" +
		// Integer.toString(mPendingSearchDirection)+ ")");

//		Global.debug("[fmradio]--->[updateStationSearch]     === mFmReceiver = " + mFmReceiver);
		if (mFmReceiver != null) {
			// Global.debug( "FmConstants.FM_COMBO_SEARCH_ENABLED
			// "+FmConstants.FM_COMBO_SEARCH_ENABLED);
			if (FmConstants.FM_COMBO_SEARCH_ENABLED) {
				...
				stationSearchUpdatePending = mSeekInProgress = (FmProxy.STATUS_OK != mFmReceiver.seekStationCombo(
						mPendingFrequency, endFrequency, FmProxy.MIN_SIGNAL_STRENGTH_DEFAULT, mPendingSearchDirection,
						mPendingScanMethod, FmConstants.COMBO_SEARCH_MULTI_CHANNEL_DEFAULT, mPendingRdsType,
	...
	private void updateLivePolling(SharedPreferences sp) {
		...
		/* Extract preferences and request these settings. */
		// mPendingLivePoll =
		// sp.getBoolean(FmRadioSettings.FM_PREF_LIVE_POLLING, false);
		// mPendingLivePollinterval =
		// Integer.parseInt(sp.getString(FmRadioSettings.FM_PREF_LIVE_POLL_INT,
		// "2000"));
	...
	private void updateSetSnrThreshold(SharedPreferences sp) {
		...
		/* Extract preferences and request these settings. */
		// mPendingSnrThreshold =
		// Integer.parseInt(sp.getString(FmRadioSettings.FM_PREF_SNR_THRESHOLD,String.valueOf(FmProxy.FM_MIN_SNR_THRESHOLD)));
	...
	private void retryPendingCommands() {
		...
		if (nflEstimateUpdatePending) {
			...
			updateFMVolume(FmProxy.FM_VOLUME_MAX * mAudioManager.getStreamVolume(AudioManager.STREAM_MUSIC)
...
/*			if (false) {
	...
	protected Handler viewUpdateHandler = new Handler() {
		...
		/**
		 * Internal helper function to update the GUI frequency display. 更新显示
		 * 
		 * @param freq
		...
		private void updateFrequency(int freq, int isCompletedSeekInt) {
			...
			// Global.debug("[updateFrequency] freq ==" + freq + "
			// isCompletedSeekInt == "+ isCompletedSeekInt );
			if (freq < 0)
				return;
		...
		public void handleMessage(Message msg) {
			...
			// Global.debug("[FmRadio]--> [handleMessage] ===msg.what = " +
			// msg.what);
			/*
...
//				Global.debug("[FmRadio]--> [handleMessage]   ===msg.what =GUI_UPDATE_MSG_FREQ_STATUS ");
			...
			case Global.GUI_UPDATE_MSG_MUTE_STATUS:
				// mView.setMutedState(msg.arg1 == FmConstants.MUTE_STATE_MUTED,
				// mInCall);
				break;
			...
			case Global.GUI_UPDTAE_LIST:
				...
				mMenuView.setSelectItem(chanel_num - 1, true);
			...
			case Global.MSG_TTSDOCOMPLETES:
				TtsUtils.getInstance().setCompletedListener(mCompletedListener);
				break;
	...
	protected class FmReceiverEventHandler implements IFmReceiverEventHandler {
		...
		/**
		 * Transfers execution of GUI function to the UI thread. 显示 频率
		 * 
		 * @param freq
		 *            the frequency (multiplied by 100) to cache and display.
		 */
		...
		/**
		 * Transfers execution of GUI function to the UI thread. 显示静音状态
		 * 
		 * @param isMute
...
//			Global.debug(
//					"[FmRadio]--->[onSeekCompleteEvent]  chanel_num ==" + chanel_num + " mSearchFlag =" + mSearchFlag);
			if ((chanel_num == 0) && (mSearchFlag == true)) { // 开始搜索
				chanel_frist = freq;
			} else {
				// int temp_freq = Integer.valueOf(chanel_di.get(0));
				int temp_freq = chanel_frist;
				// Global.debug("[FmRadio]--->[onSeekCompleteEvent] freq ==" +freq + " temp_freq =" + temp_freq + "   getChanelIsHave(freq) ="+getChanelIsHave(freq));
				 Global.debug("[FmRadio]--->[onSeekCompleteEvent] freq ==" +freq + " chanel_last =" + chanel_last);
				if (freq < chanel_last  || ((freq <= temp_freq)) /*|| true == getChanelIsHave(freq)*/) {
					mSearchFlag = false;
					chanel_last = 0;
					Message msg = Message.obtain();
			...
			/*
			 * String temp =freq/100 + "." + freq%100/10 +
			 * getResources().getString(R.string.mhz);
			 * 
			 * 
			 * if(false == getChanelIsHave(freq)){ chanel_str.add(temp);
			 * chanel_di.add(""+ freq);
			 * 
			 * setListData(ArrayUtils.strArray2ListString(chanel_str)); //
			 * mMenuView.setSelectItem(chanel_num);
			 * 
			 * chanel_num++; // 搜到的电台数 } Global.debug(
			 * "[onSeekCompleteEvent] == SIGNAL_CHECK_PENDING_EVENTS ==3==");
			 */
		...
		private boolean getChanelIsHave(int freq) {
			...
			for (i = 0; i < chanel_num; i++) {
				...
				// Global.debug("[getChanelIsHave] *** temp_freq =" + temp_freq
				// + " freq =="+ freq);
				if (freq == temp_freq) {
	...
	/*
	 * // manages frequency wrap around and scan step private void
	 * buttonSeekFrequencyUp() { Global.debug( "buttonSeekFrequencyUp: region "
	 * + mPendingRegion);
	 * 
	 * mSeekInProgress = false; // Increase the current listening frequency by
	 * one step. if (mPendingFrequency == 0) {
	 * updateFrequency(Global.DEFAULT_FREQUENCY); } else { if (mPendingFrequency
	 * < mMaxFreq) { if (mPendingFrequency % FmConstants.SCAN_STEP_100KHZ != 0)
	 * { //mPendingFrequency = (int) (mPendingFrequency +
	 * FmConstants.SCAN_STEP_50KHZ); mPendingFrequency = mPendingFrequency + 1;
	 * } else { //mPendingFrequency = (int) (mPendingFrequency +
	 * mFrequencyStep); mPendingFrequency = mPendingFrequency + 1; } } else {
	 * mPendingFrequency = mMinFreq; } updateFrequency(mPendingFrequency); } }
	 */
	/*
	 * // manages frequency wrap around and scan step private void
	 * buttonSeekFrequencyDown() { Global.debug(
	 * "buttonSeekFrequencyDown region: " + mPendingRegion);
	 * 
	 * mSeekInProgress = false; // Decrease the current listening frequency by
	 * one step. if (mPendingFrequency == 0) {
	 * updateFrequency(Global.DEFAULT_FREQUENCY); } else { if (mPendingFrequency
	 * > mMinFreq) { if (mPendingFrequency % FmConstants.SCAN_STEP_100KHZ != 0)
	 * { //mPendingFrequency = (int) (mPendingFrequency -
	 * FmConstants.SCAN_STEP_50KHZ); mPendingFrequency = mPendingFrequency - 1;
	 * } else { //mPendingFrequency = (int) (mPendingFrequency -
	 * mFrequencyStep); mPendingFrequency = mPendingFrequency - 1; } } else {
	 * mPendingFrequency = mMaxFreq; } updateFrequency(mPendingFrequency); } }
	 */

	/*
	 * public void handleButtonEvent(int buttonId, int event) { Perform the
	 * functionality linked to the activated GUI release. For each button,
	 * perform the requested action. switch (buttonId) { case
	 * FmConstants.BUTTON_POWER_OFF: Shutdown system. if (mFmReceiver != null &&
	 * mFmReceiver.getRadioIsOn() == true) { mPowerOffRadio = true;
	 * powerDownSequence(); } break;
	 * 
	 * case FmConstants.BUTTON_MUTE_ON: updateMuted(true); break;
	 * 
	 * case FmConstants.BUTTON_MUTE_OFF: updateMuted(false); break;
	 * 
	 * case FmConstants.BUTTON_TUNE_DOWN:
	 * 
	 * Scan downwards to next station. Let the FM server determine NFL for us.
	 * 
	 * updateStationSearch(FmProxy.SCAN_MODE_DOWN); break;
	 * 
	 * case FmConstants.BUTTON_TUNE_UP:
	 * 
	 * Scan upwards to next station. Let the FM server determine NFL for us.
	 * 
	 * updateStationSearch(FmProxy.SCAN_MODE_UP); break;
	 * 
	 * case FmConstants.BUTTON_SEEK_DOWN: Decrease the current listening
	 * frequency by one step. buttonSeekFrequencyDown(); break;
	 * 
	 * case FmConstants.BUTTON_SEEK_UP: Increase the current listening frequency
	 * by one step. buttonSeekFrequencyUp(); break;
	 * 
	 * case FmConstants.BUTTON_SETTINGS: // openOptionsMenu(); Intent intent =
	 * new Intent(); intent.setClass(FmRadio.this, FmRadioSettings.class);
	...
	/*
	 * // 显示通知栏 private void showNotification() { int icon = R.drawable.icon;
	 * String tickerText = String.format("FM Radio (%.02f MHz)", ((double)
	 * this.mPendingFrequency) / 100); long when = System.currentTimeMillis();
	 * if (mInCall) tickerText += " (in call - muted)"; Notification
	 * notification = new Notification(icon, tickerText, when);
	 * 
	 * Context context = getApplicationContext(); String contentTitle =
	 * "FmRadio app"; String contentText = String.format("%.02f MHz", ((double)
	 * this.mPendingFrequency) / 100); if (mInCall) contentTitle +=
	 * " (in call - muted)"; Intent notificationIntent = new Intent(this,
	 * FmRadio.class); PendingIntent contentIntent =
	 * PendingIntent.getActivity(this, 0, notificationIntent, 0);
	 * 
	 * notification.setLatestEventInfo(context, contentTitle, contentText,
	 * contentIntent);
	 * 
	 * final int PLAYING_ID = 1; // mNotificationManager.notify(PLAYING_ID,
	 * notification); }
	 */
	 ...
	 * 
	 * //SunteamToast mSunteamToast = new SunteamToast(this); if (speakerFlag) {
	 * audioManager.setWiredDeviceConnectionState(8, 55, "h2w"); // close
	 * headset and open speak //mSunteamToast.show("设置外放"); } else {
	 * audioManager.setWiredDeviceConnectionState(8, 66, "h2w"); // close speak
	 * //mSunteamToast.show("关闭外放"); } speakerFlag = !speakerFlag; }
	 */
	private void setWiredDeviceConnectionState(Boolean flag) {
		...
		try {
			...
			if (flag) {
				method.invoke(audioManager, 8, 55, "h2w"); // close headset and
															// open speak
			} else {
	...
	private void setHearToSpeak(Boolean flag) {
		...
		if (true == flag) { // 切换到外放
			try {
				...
				method.invoke(audioManager, 8, 55, "h2w"); // close headset and
															// open speak
				// method.invoke(audioManager, 8, 0, "h2w"); // close headset
				// and open speak
...
//				 Global.debug("\r\n[BatteryBroadcastReciver] isCharging == " + isCharging + "  status ="+status + "   level=="+level);
				if (Test == true) {
					// level = 5;
				}
				if (isCharging == true) {
				...
				if (level < 10) {
					...
					// Global.showToast(PlayActivity.this,
...
//			return ;
			Global.debug("\r\n tts 播放结束========mVolFlag ="+ mVolFlag + "   mSearchFlag ="+mSearchFlag);
			if (mVolFlag == true) {
				updateMuted(false);
				return;
			}
