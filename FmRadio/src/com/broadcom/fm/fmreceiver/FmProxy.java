...
public final class FmProxy {
    ...
    public static final int FM_VOLUME_MAX = 255;   // 音量最大255
    ...
    protected boolean init(IBinder service) {
        try {
            mService = IFmReceiverService.Stub.asInterface(service);
            return true;
        } catch (Throwable t) {
    ...
    public synchronized void registerEventHandler(IFmReceiverEventHandler handler) {
        Log.v(TAG, "registerEventHandler()555555555555555555555");
        Global.debug("registerEventHandlerb  ===============================");
    ...
    public synchronized int tuneRadio(int freq) {
        ...
        Global.debug("[tuneRadio] ======freq ==== " + freq + "\r\n returnCode ===" + returnCode);
        return returnCode;
    }
    ...
    public synchronized int seekStationCombo(int startFrequency, int endFrequency,
       ...
       Global.debug("seekStationCombo() seekStationComb 1111111=====");
        try {
            ...
            Global.debug("seekStation() failed====$$$$$$$$$$");
        }
        
        Global.debug("seekStationCombo() returnCode　＝＝　"+returnCode);
        return returnCode;
    }
